"""
SimpleBootstrapper: reads research.md + stack.md, generates the full project structure.
Uses Jinja2 templates for agents, skills, context.md and manifest.json.

This is the legacy v0.3 bootstrapper, refactored for simple mode in Phase 3.
"""
...
from ...core.base_bootstrapper import BaseBootstrapper
from ...core.config import VibecraftConfig

console = Console()

# Templates are located in vibecraft/templates (shared across modes)
...
class SimpleBootstrapper(BaseBootstrapper):
    """
    Legacy v0.3 bootstrapper for simple mode projects.
    
    This class maintains backward compatibility with v0.3 while
    inheriting from BaseBootstrapper for Phase 2+ architecture.
    """
    ...
    def __init__(
        ...
        """
        Initialize the SimpleBootstrapper.
        
        Args:
            project_root: Root directory of the project.
            config: VibecraftConfig object.
            research_path: Path to research.md (optional, uses config if not provided).
            stack_path: Path to stack.md (optional, uses config if not provided).
            custom_agents_path: Path to custom agents.yaml (optional).
    ...
    def validate(self) -> list[str]:
        ...
        research_stripped = self.research_content.strip() if self.research_content else ""
        stack_stripped = self.stack_content.strip() if self.stack_content else ""
    ...
    def _build_context(self) -> dict:
        project_name = self._extract_project_name() or (
            self.research_path.stem.replace("_", " ").title() if self.research_path else "Project"
        )
    ...
    def _load_custom_agents(self, agents: list[str]) -> list[str]:
        # Check if custom_agents_path is provided
    ...
    def _create_dirs(self, ctx: dict):
        ...
        dirs = [
            ...
            root / ".vibecraft" / "prompts",
            root / ".vibecraft" / "snapshots",
            root / "docs" / "design",
            root / "docs" / "plans",
            root / "src" / "tests",
        ]
    ...
    def _copy_inputs(self, ctx: dict):
        if not self.research_path or not self.stack_path:
            return
        ...
        def _safe_copy(src: Path, dst: Path, label: str):
            ...
            if dst.exists() and not self.force:
                console.print(f"  [yellow]  WARN: {dst.relative_to(self.project_root)} already exists.[/yellow]")
                try:
        ...
        if self.custom_agents_path and self.custom_agents_path.exists():
            shutil.copy(
                ...
                self.project_root / ".vibecraft" / "custom_agents.yaml",
            )
            console.print("  [dim]→ .vibecraft/custom_agents.yaml[/dim]")

    def _generate_agents(self, ctx: dict):
        agents_dir = self.project_root / ".vibecraft" / "agents"
    ...
    def _generate_skills(self, ctx: dict):
        skills_dir = self.project_root / ".vibecraft" / "skills"
    ...
    def _generate_manifest(self, ctx: dict):
        ...
        out_path = self.project_root / ".vibecraft" / "manifest.json"
        out_path.write_text(
